...
#
# Ruby 3.1's YJIT is the C implementation: there is no Rust toolchain involved
# and 3.1.7's configure has no --enable-yjit flag. The without-yjit option is
# kept (inert for codegen) so the release build matrix works unchanged; it
# still selects the glibc@2.17 toolchain on Linux.
class JdxRuby31 < Formula
 ...
 def install
    ...
    # 3.1's Makefile has no ruby.pc alias target; pkgconfig-data builds ruby-3.1.pc
